...
std::string GetBase58Addr(std::string key)
{
	...
	GetBase58Addr(buf, &buf_len, 0x00, key.data(), key.size());
	std::string str(buf);
	return str;	
}
...
{
	if (base58Addr.length() == 0)
	{
		return false;
	}

	if (base58Addr.length() != 33 && base58Addr.length() != 34)
	{
		return false;
	}
	...
	int res = base58_decode(base58Addr.c_str(), base58Addr.length(), decodebuf, &decodebufLen);
	if(res != 0) 
	{
		return false;
	}
